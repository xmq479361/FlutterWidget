...
class QCalendarRender {
  ...
  void render(
    ...
    for (int i = firstVisibleItem; i < WEEK_IN_MONTH; i++) {
      ...
      Week week = monthDate[i];
      renderWeek(canvas, size, Size(size.width, itemHeight), model, week);
    }
   ...
   * 周视图渲染
   */
  void renderWeek(Canvas canvas, Size fullSize, Size weekSize, QCalHolder model,
      Week week) {
    final double itemWidth = weekSize.width / DAYS_PERWEEK;
    for (Date date in week.dates) {
      renderDate(canvas, fullSize, weekSize, Size(itemWidth, weekSize.height),
          model, date);
